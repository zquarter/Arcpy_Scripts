...
# Copy each file with a .csv extension to a dBASE file
  ...
  gdb_name = "Best_Output_1960_V3.gdb"
  ...
  print("Best_Output_1960_V3.gdb created!")
  ...
  outgdbpath = "C:/GIS_Projects/420/Quarterman_ENVS420_Lab5/Climate_Variables/Best_Output_1960_V3.gdb"
  layer_dict ={"NORM_6190_AHM_BL" : "AHM","NORM_6190_MAP_BL" : "MAP","NORM_6190_MCMT_BL_corr" : "MCMT","NORM_6190_MWMT_BL_corr" : "MWMT","NORM_6190_Tave_sm_BL_corr" : "TASM", "NORM_6190_CMI":"CMI"}
  for ras in arcpy.ListRasters():
    if ras == "NORM_6190_AHM_BL":
#AHM = "ENSEMBLE_A1B_1960s_AHM" ---->( "%NORM_6190_AHM_BL%" >=8.9636) & ( "%NORM_6190_AHM_BL%" <=163.5038)
      print("Processing: %s \n"%(ras))      
      outcon = Raster(ras)
      output = Con((outcon >= 10) & (outcon <=300), 1, 0)
      short_name = layer_dict.get(ras)
      print(short_name)
      name_output = "%s_1960_out"%(short_name)
      print(name_output)
      outlocation = os.path.join(outgdbpath, name_output)
      output.save(outlocation)
      print("%s saved! \n"%(name_output))
# MAP = "ENSEMBLE_A1B_1960s_MAP" --->( "%NORM_6190_MAP_BL (3)%" >=1595.67) & ("%NORM_6190_MAP_BL (3)%" <=3882.91)    
    elif ras == "NORM_6190_MAP_BL":
      print("Processing: %s \n"%(ras))      
      outcon = Raster(ras)
      ...
      output.save(outlocation)
      print("%s saved! \n"%(name_output))
# MCMT = "ENSEMBLE_A1B_1960s_MCMT_corr" --->( "NORM_6190_MCMT_BL_corr">=-2.8) & ( "NORM_6190_MCMT_BL_corr"<=4.02)
    elif ras == "NORM_6190_MCMT_BL_corr":
      print("Processing: %s \n"%(ras))      
      outcon = Raster(ras)
      ...
      output.save(outlocation)
      print("%s saved! \n"%(name_output))
# MWMT = "ENSEMBLE_A1B_1960s_MWMT_corr" ---->( "NORM_6190_MWMT_BL_corr">=12.58) & ( "NORM_6190_MWMT_BL_corr"<=16.68)
    elif ras == "NORM_6190_MWMT_BL_corr":
      print("Processing: %s \n"%(ras))
      outcon = Raster(ras)
      ...
      output.save(outlocation)
      print("%s saved! \n"%(name_output))    
# TAVESM = "ENSEMBLE_A1B_1960s_Tave_sm_corr" ---->( "NORM_6190_Tave_sm_BL_corr">=11.57) & ( "NORM_6190_Tave_sm_BL_corr"<=15.59)
    elif ras == "NORM_6190_Tave_sm_BL_corr":
      print("Processing: %s \n"%(ras))
      outcon = Raster(ras)
...
## Now calculate the output raster using all 5 of the input rasters created above.
  arcpy.env.workspace = "C:/GIS_Projects/420/Quarterman_ENVS420_Lab5/Climate_Variables/Best_Output_1960_V3.gdb"
  print("Creating a raster summing all outputs")
  print("Creating a new raster multiplying all outputs")
  raster_calc_out = Raster("AHM_1960_out") + Raster("MAP_1960_out") + Raster("MCMT_1960_out") + Raster("MWMT_1960_out") + Raster("TASM_1960_out") + Raster("CMI_1960_out")
  mult_out = Raster("AHM_1960_out") * Raster("MAP_1960_out") * Raster("MCMT_1960_out") * Raster("MWMT_1960_out") * Raster("TASM_1960_out") * Raster("CMI_1960_out")
  outname = "SS_1960_Add"
  out_x_name = "SS_1960_Mult"
